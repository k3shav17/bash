...
## LOOPING ONE TIME FOR GENERATING THE PASSWORD. YOU CAN CHANGE THE NUMBER IN THE SEQ TO ANY NUMBER WHICH GENERATES THAT NUMBER OF PASSWORDS, FROM WHICH YOU CAN SELECT.
for p in $(seq 1);
do
	FIRSTHALF=`openssl rand -base64 48 | cut -c5-$PASSLENG` 
	SECONDHALF=`openssl rand -base64 48 | cut -c1-$PASSLENG`
done

## STORING THE PASSWORDS TO A TEXT FILE FOR LATER REFERENCE
echo "$SITENAME:	^$FIRSTHALF$SECONDHALF!" >> ~/Documents/passwords.txt
